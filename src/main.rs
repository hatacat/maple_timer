...
impl Default for MatchOptions {
    fn default() -> Self {
        Self {
            jinhillah: false,
            jinhillah_hard: true,
            vskill: (false,false,false,false),
...
프로그램을 재시작해주세요.",
        ...
        ui.allocate_ui_with_layout(
            ...
            |ui| {
                ui.horizontal_wrapped(|ui| {
                    ...
                    let something = self.match_options.jinhillah || self.match_options.vskill.0 || self.match_options.vskill.1 || self.match_options.vskill.2 || self.match_options.vskill.3 ;
        ...
        ui.add_enabled_ui(self.timers.is_empty(), |ui| {
            ...
            */
            ui.horizontal_wrapped(|ui| { // 메르 쿨감
                ui.selectable_value(
                    &mut self.match_options.union_cooldown,
                    4.,
                    "메르 140"
                );
                ui.selectable_value(
            ...
            ui.horizontal_wrapped(|ui| { // 쿨감뚝
                ui.selectable_value(
                    &mut self.match_options.cooldown,
                    0.,
                    "0초뚝"
                );
                ui.selectable_value(
            ...
            ui.horizontal_wrapped(|ui| {
                ui.checkbox(
                    &mut self.match_options.vskill.0,
                    "일격필살",
                );
                ui.checkbox(
                    &mut self.match_options.vskill.1,
                    "오버드라이브",
                );
                ui.checkbox(
                    &mut self.match_options.vskill.2,
                    "스포트라이트",
                );
                ui.checkbox(
                    &mut self.match_options.vskill.3,
                    "보스슬레이어",
                );
            });
        });
    ...
    fn init_timers(&mut self) {
        ...
        if self.match_options.vskill.0 {
            self.timers.push(Box::new(VSkillTimer::new(
                Arc::clone(self.capturer.as_mut().unwrap().as_mut().unwrap().cond()),
                Arc::clone(self.capturer.as_mut().unwrap().as_mut().unwrap().lock_ref()),
                VSkillKind::FatalStrike,
                self.capturer.as_mut().unwrap().as_mut().unwrap().dims(),
                self.match_options.union_cooldown,
                self.match_options.cooldown,
                "v_buficon",
                0.99,
            )))
        }

        if self.match_options.vskill.1 {
            self.timers.push(Box::new(VSkillTimer::new(
                Arc::clone(self.capturer.as_mut().unwrap().as_mut().unwrap().cond()),
                Arc::clone(self.capturer.as_mut().unwrap().as_mut().unwrap().lock_ref()),
                VSkillKind::Overdrive,
                self.capturer.as_mut().unwrap().as_mut().unwrap().dims(),
                self.match_options.union_cooldown,
                self.match_options.cooldown,
                "overdrive",
        ...
        if self.match_options.vskill.2 {
            self.timers.push(Box::new(VSkillTimer::new(
                Arc::clone(self.capturer.as_mut().unwrap().as_mut().unwrap().cond()),
                Arc::clone(self.capturer.as_mut().unwrap().as_mut().unwrap().lock_ref()),
                VSkillKind::Spotlight,
                self.capturer.as_mut().unwrap().as_mut().unwrap().dims(),
                self.match_options.union_cooldown,
                self.match_options.cooldown,
                "spotlight",
        ...
        if self.match_options.vskill.3 {
            self.timers.push(Box::new(VSkillTimer::new(
                Arc::clone(self.capturer.as_mut().unwrap().as_mut().unwrap().cond()),
                Arc::clone(self.capturer.as_mut().unwrap().as_mut().unwrap().lock_ref()),
                VSkillKind::Bossslayer,
                self.capturer.as_mut().unwrap().as_mut().unwrap().dims(),
                self.match_options.union_cooldown,
                self.match_options.cooldown,
                "bossslayer",
